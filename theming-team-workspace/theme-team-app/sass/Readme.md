...
folder are mapped to the application's root namespace, 'ThemeTeamApp'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ThemeTeamApp"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ThemeTeamApp' in the same way
as `"ThemeTeamApp/sass/src"`.
