    ...
    onTopicTeamShow: function(){
        ...
        if ( $( "#canvas" ).length ) { 
            ...
            if (topicStore.data.items.length > 0) {
                topicStore.data.items.forEach(function(entry) {
                    if (entry.data.team_id == null) {
                        tmpObject = {'fillStyle' : arrayColorSegments[indexColor], 'text' : entry.data.name};
            ...
            arrayTeam.forEach(function(entry) {
                optionList = optionList + '<div class="single-option">\
                        <input type="radio" id="team-radio-wheel-' + entry._id + '" name="selectTeam" />\
