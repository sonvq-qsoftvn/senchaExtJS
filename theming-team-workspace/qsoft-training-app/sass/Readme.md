...
folder are mapped to the application's root namespace, 'QsoftTrainingApp'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "QsoftTrainingApp"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'QsoftTrainingApp' in the same way
as `"QsoftTrainingApp/sass/src"`.
